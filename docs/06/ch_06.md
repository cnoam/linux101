...
# What are 'filter programs'?

Linux filter programs are modular command-line utilities that read text from standard input, transform it, and output the result. Following the "do one thing and do it well" philosophy, tools like **grep**, **awk**, and **sort** are designed to be chained together using pipes (`|`). This allows you to build custom, complex data pipelines where the processed output of one command flows seamlessly into the next, making them indispensable for efficient log analysis and text manipulation.
